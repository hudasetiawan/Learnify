    ...
    <article class="w-full flex flex-col gap-16">
        ...
        <div class="w-full px-4 py-8">
            <form action="" method="POST" class="bg-white shadow-md rounded-lg p-6">
                @csrf
                <h2 class="text-2xl font-bold mb-6 text-gray-800 text-center">Create New Quiz Question</h2>
                ...
                <div class="mb-4">
                    ...
                    <textarea id="question" name="question" rows="4"
                        ...
                        placeholder="Enter your quiz question here" required></textarea>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    @foreach ($choices as $choice)
                        <div>
                            <label for="choice_{{ strtolower($choice) }}" class="block text-gray-700 font-medium mb-2">
                                Choice {{ $choice }}
                            </label>
                            <input type="text" id="choice_{{ strtolower($choice) }}"
                                name="choices[{{ strtolower($choice) }}]"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                placeholder="Enter choice {{ $choice }}" required>
                        </div>
                    @endforeach
                </div>

                <div class="mb-4">
                    <label for="correct_answer" class="block text-gray-700 font-medium mb-2">Correct Answer</label>
                    <select id="correct_answer" name="correct_answer"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required>
                        <option value="">Select Correct Answer</option>
                        @foreach ($choices as $choice)
                            <option value="{{ strtolower($choice) }}">Choice {{ $choice }}</option>
                        @endforeach
                    </select>
                </div>
